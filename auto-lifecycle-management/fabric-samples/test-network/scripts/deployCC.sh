...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.automobile-lifecycle.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.automobile-lifecycle.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.automobile-lifecycle.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
installChaincode "dealer"
infoln "Install chaincode on peer0.regulator..."
installChaincode "regulator"
infoln "Install chaincode on peer0.vehicleowner..."
installChaincode "vehicleowner"
infoln "Install chaincode on peer0.carserviceprovider..."
installChaincode "carserviceprovider"
infoln "Install chaincode on peer0.insuranceprovider..."
installChaincode "insuranceprovider"
infoln "Install chaincode on peer0.scrapmerchant..."
installChaincode "scrapmerchant"
...
approveForMyOrg "dealer"
approveForMyOrg "regulator"
approveForMyOrg "vehicleowner"
approveForMyOrg "carserviceprovider"
approveForMyOrg "insuranceprovider"
approveForMyOrg "scrapmerchant"

## check whether the chaincode definition is ready to be committed
## expect org1 to have approved and org2 not to
infoln "Checking commit readiness"
checkCommitReadiness "manufacturer"
checkCommitReadiness "dealer"
checkCommitReadiness "regulator"
checkCommitReadiness "vehicleowner"
checkCommitReadiness "carserviceprovider"
checkCommitReadiness "insuranceprovider"
checkCommitReadiness "scrapmerchant"

## now that we know for sure both orgs have approved, commit the definition
commitChaincodeDefinition "manufacturer" "dealer" "regulator" "vehicleowner" "carserviceprovider" "insuranceprovider" "scrapmerchant"

## query on both orgs to see that the definition committed successfully
infoln "Query the committed"
queryCommitted "manufacturer"
queryCommitted "dealer"
queryCommitted "regulator"
queryCommitted "vehicleowner"
queryCommitted "carserviceprovider"
queryCommitted "insuranceprovider"
queryCommitted "scrapmerchant"
...
if [ "$CC_INIT_FCN" = "NA" ]; then
  ...
  chaincodeInvokeInit "manufacturer" "dealer" "regulator" "vehicleowner" "carserviceprovider" "insuranceprovider" "scrapmerchant"
fi
